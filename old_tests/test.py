...
def main():
    ...
    traj.f_add_parameter('test.testparam', **{'Fuechse':np.array([[1,2],[3.3,2]]),'Sapiens':1,'Comment':'ladida'})
    ...
    traj.Parameters.test.testparam.f_set({'Madam':'is_broke', 'Papa' : 0.0})
    
    traj.Parameters.test.testparam.f_set('Fuechse', np.array([1,2,3.6]))
    ...
    print traj.Parameters.test.testparam.f_to_dict()
    
    print traj.Parameters.test.testparam.to_dict_of_lists()
    
    print traj.Parameters.test.testparam.to_list_of_dicts()
    
    print traj.Parameters.test.testparam.f_get_class_name()
    
    lilma = spsp.lil_matrix((10000,1000))
    lilma[0,100] = 555
    lilma[9999,999] = 11
    traj.f_add_parameter('test.sparseparam', Fuechse2=lilma, param_type=SparseParameter)
    
    print traj.Parameters.test.sparseparam.f_get_class_name()
    
    
    traj.f_add_derived_parameter('test.sparseparam', Fuechse2=lilma*2, param_type=SparseParameter)
