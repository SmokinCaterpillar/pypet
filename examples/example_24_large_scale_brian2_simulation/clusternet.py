...
import numpy as np
from brian2 import Equations, NeuronGroup, SpikeMonitor, StateMonitor, Synapses, ms, rand

from pypet.brian2.network import NetworkAnalyser, NetworkComponent, NetworkRunner
...
class CNNeuronGroup(NetworkComponent):
    ...
    def add_parameters(traj):
        ...
        traj.f_add_parameter(
            "model.eqs", model_eqs, comment="The differential equation for the neuron model"
        )
        ...
        traj.f_add_parameter(
            "model.reset_func", "V=0.0", comment="String representation of reset function"
        )
...
class CNConnections(NetworkComponent):
    ...
    def add_parameters(traj):
        ...
        traj.f_add_parameter(
            "connections.strength_factor", 2.5, comment="Factor for scaling cluster weights"
        )

        traj.f_add_parameter(
            "connections.p_ii", 0.25, comment="Connection probability from inhibitory to inhibitory"
        )
        traj.f_add_parameter(
            "connections.p_ei", 0.25, comment="Connection probability from inhibitory to excitatory"
        )
        traj.f_add_parameter(
            "connections.p_ie", 0.25, comment="Connection probability from excitatory to inhibitory"
        )
        traj.f_add_parameter(
            "connections.p_ee", 0.1, comment="Connection probability from excitatory to excitatory"
        )
    ...
    def pre_build(self, traj, brian_list, network_dict):
        ...
        self._pre_build = (
            self._pre_build and "neurons_i" in network_dict and "neurons_e" in network_dict
        )
    ...
    def _build_connections(self, traj, brian_list, network_dict):
        ...
        if connections.R_ee > 1.0:
            ...
            traj.f_add_derived_parameter(
                "connections.clusters", clusters, comment="Number of clusters"
            )
            ...
            traj.f_add_derived_parameter(
                "connections.p_ee_in", p_in, comment="Connection prob within cluster"
            )
            traj.f_add_derived_parameter(
                "connections.p_ee_out", p_out, comment="Connection prob to outside of cluster"
            )
            ...
            for irun in range(clusters):
                ...
                if low_index > 0:
                    ...
                    low_conn = Synapses(cluster, rest_low, on_pre="y_e += %f" % connections.J_ee)
                ...
                if high_index < model.N_e:
                    ...
                    high_conn = Synapses(cluster, rest_high, on_pre="y_e += %f" % connections.J_ee)
            ...
            self.conn_ee = Synapses(neurons_e, neurons_e, on_pre="y_e += %f" % connections.J_ee)
...
class CNNetworkRunner(NetworkRunner):
    ...
    def add_parameters(self, traj):
        ...
        par = traj.f_add_parameter(
            Brian2Parameter,
            "simulation.durations.initial_run",
            500 * ms,
            comment="Initialisation run for more realistic measurement conditions.",
        )

        par.v_annotations.order = 0
        par = traj.f_add_parameter(
            Brian2Parameter,
            "simulation.durations.measurement_run",
            1500 * ms,
            comment="Measurement run that is considered for statistical evaluation",
        )
...
class CNFanoFactorComputer(NetworkAnalyser):
    ...
    def add_parameters(self, traj):
        traj.f_add_parameter(
            "analysis.statistics.time_window", 100 * ms, "Time window for FF computation"
        )
        traj.f_add_parameter(
            "analysis.statistics.neuron_ids",
            tuple(range(500)),
    ...
    def analyse(self, traj, network, current_subrun, subrun_list, network_dict):
        ...
        if len(subrun_list) == 0:
            ...
            traj.f_add_result(
                "statistics.mean_fano_factor",
                mean_ff,
                comment="Average Fano Factor over all exc neurons",
            )
...
class CNMonitorAnalysis(NetworkAnalyser):
    ...
    def add_parameters(traj):
        traj.f_add_parameter(
            "analysis.neuron_records", (0, 1, 100, 101), comment="Neuron indices to record from."
        )
        traj.f_add_parameter(
            "analysis.plot_folder",
    ...
    def _add_monitors(self, traj, network, network_dict):
        ...
        # Membrane Potential
        self.V_monitor = StateMonitor(neurons_e, "V", record=list(traj.neuron_records))

        monitor_list.append(self.V_monitor)

        # Exc. syn .Current
        self.I_syn_e_monitor = StateMonitor(neurons_e, "I_syn_e", record=list(traj.neuron_records))
        monitor_list.append(self.I_syn_e_monitor)

        # Inh. syn. Current
        self.I_syn_i_monitor = StateMonitor(neurons_e, "I_syn_i", record=list(traj.neuron_records))
    ...
    def _make_folder(self, traj):
        ...
        """
        print_folder = os.path.join(traj.analysis.plot_folder, traj.v_name, traj.v_crun)
