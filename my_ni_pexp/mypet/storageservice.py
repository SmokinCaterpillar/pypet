...
class HDF5StorageService(object):
    ...
    def _store_parameter(self, param, traj):
        ...
        for key, list_of_dicts in store_dict:
            _store_into_a_pytable(key, list_of_dicts, group, fullname)
    ...
    def _store_into_pytable(self,tablename,data,hdf5group,fullname):
        ...
        description_dict = self._make_description(data[0],fullname, data, [])

        table = self._hdf5file.create_table(hdfgroup,tablename,description_dict)
    ...
    def _make_description(self, data_dict, fullname, data, previous_keys):
        ...
        for key, val in data_dict.items():
            ...
            if isinstance(val, dict):
                col = self._make_description(val, fullname, data, previous_keys[:].append(key))
            else:
                col = self._get_table_col(key, val, data, previous_keys)
    ...
    def _get_table_col(self, key, val, data, previous_keys):
        ...
        if isinstance(val, np.str):
                itemsize = int(self._get_longest_stringsize(key,data,previous_keys))
    ...
    def _get_longest_stringsize(self,key, data, previous_keys):
