...
from pandas import DataFrame, HDFStore, Series, StringDtype
...
from pypet.pypetlogging import DisableAllLogging, HasLogger
...
class HDF5StorageService(StorageService, HasLogger):
    ...
    ATTR_LIST = ["complevel", "complib", "shuffle", "fletcher32", "pandas_format", "encoding"]
    ...
    def __init__(
        ...
        if purge_duplicate_comments and not summary_tables:
            raise ValueError(
                "You cannot purge duplicate comments without having the small overview tables."
            )
        ...
        if overwrite_file:
            try:
                os.remove(filename)
                self._logger.info(
                    f"You specified ``overwrite_file=True``, so I deleted the file `{filename}`."
                )
            except OSError:
    ...
    def _all_get_filters(self, kwargs=None):
        ...
        if self._filters is None:
            ...
            self._filters = pt.Filters(
                complib=complib, complevel=complevel, shuffle=shuffle, fletcher32=fletcher32
            )
    ...
    def _srvc_set_config(self, trajectory):
        ...
        for attr_name in HDF5StorageService.NAME_TABLE_MAPPING:
            ...
            _set_config(
                "hdf5.overview." + table_name,
                value,
                comment="Whether or not to have an overview table with that name",
            )
        ...
        _set_config(
            "hdf5.complevel",
            self._complevel,
            comment="Compression Level (0 no compression to 9 highest compression)",
        )

        _set_config("hdf5.complib", self._complib, comment="Compression Algorithm")

        _set_config("hdf5.encoding", self._encoding, comment="Encoding for unicode characters")

        _set_config(
            "hdf5.fletcher32", self._fletcher32, comment="Whether to use fletcher 32 checksum"
        )

        _set_config("hdf5.shuffle", self._shuffle, comment="Whether to use shuffle filtering.")
    ...
    def _srvc_check_hdf_properties(self, traj):
        ...
        for attr_name in HDF5StorageService.ATTR_LIST:
            try:
                ...
                self._logger.debug(
                    f"Could not find `{attr_name}` in traj config, "
        ...
        for attr_name, table_name in HDF5StorageService.NAME_TABLE_MAPPING.items():
            try:
                ...
                self._logger.debug(
                    f"Could not find `{table_name}` in traj config, "
                    f"using (default) value `{getattr(self, attr_name)}`."
                )
        ...
        for attr_name, name in HDF5StorageService.PR_ATTR_NAME_MAPPING.items():
            try:
                ...
                self._logger.debug(
                    f"Could not find `{name}` in traj config, "
                    f"using (default) value `{getattr(self, attr_name)}`."
                )

        if (
            not self._overview_results_summary or not self._overview_derived_parameters_summary
        ) and self._purge_duplicate_comments:
            raise RuntimeError(
                "You chose to purge duplicate comments but disabled a summary "
        ...
        if not self.is_open:
            if "a" in mode:
                ...
                if self._trajectory_name is not None:
                    if "/" + self._trajectory_name not in self._hdf5file:
                        ...
                        if not msg == pypetconstants.TRAJECTORY:
                            raise ValueError(
                                "Your trajectory cannot be found in the hdf5file, "
                ...
                self._logger.debug(
                    f"Opening HDF5 file `{self._filename}` in mode `a` with trajectory `{self._trajectory_name}`"
                )

            elif mode == "r":
                if self._trajectory_name is not None and self._trajectory_index is not None:
                    raise ValueError(
                        "Please specify either a name of a trajectory or an index, "
                ...
                if self._trajectory_index is not None:
                    ...
                    if self._trajectory_index >= len(nodelist) or self._trajectory_index < -len(
                        nodelist
                    ):
                        raise ValueError(
                            f"Trajectory No. {self._trajectory_index} does not exists, there are only "
                    ...
                    if "/" + self._trajectory_name not in self._hdf5file:
                        raise ValueError(
                            f"File {self._filename} does not contain trajectory {self._trajectory_name}."
                        )

                    self._trajectory_group = self._hdf5file.get_node("/" + self._trajectory_name)
                else:
                    raise ValueError(
                        "Please specify a name of a trajectory to load or its "
                        "index, otherwise I cannot open one."
                    )

                self._logger.debug(
                    f"Opening HDF5 file `{self._filename}` in mode `r` with trajectory `{self._trajectory_name}`"
                )

            else:
                raise RuntimeError("You shall not pass!")

            self._node_processing_timer = NodeProcessingTimer(
                display_time=self._display_time, logger_name=self._logger.name
            )
    ...
    def _trj_backup_trajectory(self, traj, backup_filename=None):
        ...
        if "/" + self._trajectory_name in backup_hdf5file:
            raise ValueError(
                f"I cannot backup  `{traj.v_name}` into file `{backup_filename}`, there is already a "
    ...
    def _trj_merge_trajectories(
        ...
        try:
            if "/" + other_trajectory_name not in other_file:
                raise ValueError(
                    f"Cannot merge `{self._trajectory_name}` and `{other_trajectory_name}`, because the second trajectory cannot "
            ...
            for old_name in rename_dict:
                ...
                if move_nodes:
                    self._hdf5file.move_node(
                        where=old_node,
                        newparent=new_parent_location,
                        newname=new_short_name,
                        createparents=True,
                    )
                else:
                    if other_is_different:
                        ...
                        create_parents = True
                    self._hdf5file.copy_node(
                        where=old_node,
                        newparent=new_parent_or_loc,
                        newname=new_short_name,
    ...
    def _trj_prepare_merge(self, traj, changed_parameters, old_length):
        ...
        self._all_add_or_modify_row(
            traj.v_name, insert_dict, infotable, index=0, flags=(HDF5StorageService.MODIFY_ROW,)
        )
        ...
        for idx in range(old_length, len(traj)):
            ...
            run_summary = self._srn_summarize_explored_parameters(
                list(traj._explored_parameters.values())
            )

            run_info["parameter_summary"] = run_summary

            self._all_add_or_modify_row(
                run_name, run_info, run_table, index=idx, flags=(HDF5StorageService.MODIFY_ROW,)
            )
    ...
    def _trj_load_trajectory(
        ...
        if as_new and (
            ...
            or load_results != pypetconstants.LOAD_NOTHING
            or load_other_data != pypetconstants.LOAD_NOTHING
        ):
            raise ValueError(
        ...
        if as_new and load_parameters != pypetconstants.LOAD_DATA:
            raise ValueError(
                "You cannot load the trajectory as new and not load the data of the parameters."
            )

        loadconstants = (
            pypetconstants.LOAD_NOTHING,
            pypetconstants.LOAD_SKELETON,
            pypetconstants.LOAD_DATA,
            pypetconstants.OVERWRITE_DATA,
        )

        if not (
            load_parameters in loadconstants
            and load_derived_parameters in loadconstants
            and load_results in loadconstants
            and load_other_data in loadconstants
        ):
            raise ValueError(
                f"Please give a valid option on how to load data. Options for "
        ...
        if (
            load_parameters != pypetconstants.LOAD_NOTHING
            or load_derived_parameters != pypetconstants.LOAD_NOTHING
            or load_results != pypetconstants.LOAD_NOTHING
            or load_other_data != pypetconstants.LOAD_NOTHING
        ):
    ...
    def _srvc_load_hdf5_settings(self):
        ...
        if "hdf5_settings" in self._overview_group:
            ...
            _extract_meta_data("_results_per_run", hdf5_row, "results_per_run", int)
            _extract_meta_data(
                "_derived_parameters_per_run", hdf5_row, "derived_parameters_per_run", int
            )
            _extract_meta_data(
                "_purge_duplicate_comments", hdf5_row, "purge_duplicate_comments", bool
            )
    ...
    def _tree_load_sub_branch(
        ...
        if _hdf5_group is None:
            ...
            if hdf5_group_name == "":
                ...
                try:
                    _hdf5_group = self._hdf5file.get_node(
                        where=self._trajectory_group, name=hdf5_group_name
                    )
                except pt.NoSuchNodeError:
                    self._logger.error(
                        f"Cannot find `{traj_node.v_full_name}` the hdf5 node `{hdf5_group_name}` does not exist!"
                    )
                    raise
    ...
    def _trj_check_version(self, version, python, force):
        ...
        if (version != VERSION or curr_python != python) and not force:
            raise pex.VersionMismatchError(
                f"Current pypet version is {VERSION} used under python {curr_python} "
                "  but your trajectory"
            ...
            self._logger.warning(
                f"Current pypet version is {VERSION} with python {curr_python} but your trajectory"
    ...
    def _trj_store_meta_data(self, traj):
        ...
        descriptiondict = {
            "name": pt.StringCol(pypetconstants.HDF5_STRCOL_MAX_LOCATION_LENGTH, pos=0),
        ...
        infotable = self._all_get_or_create_table(
        ...
        rundescription_dict = {
            "name": pt.StringCol(pypetconstants.HDF5_STRCOL_MAX_NAME_LENGTH, pos=1),
            "time": pt.StringCol(len(traj.v_time), pos=2),
            "timestamp": pt.FloatCol(pos=3),
            "idx": pt.IntCol(pos=0),
            "completed": pt.IntCol(pos=8),
            "parameter_summary": pt.StringCol(pypetconstants.HDF5_STRCOL_MAX_COMMENT_LENGTH, pos=6),
            "short_environment_hexsha": pt.StringCol(7, pos=7),
            "finish_timestamp": pt.FloatCol(pos=4),
            "runtime": pt.StringCol(pypetconstants.HDF5_STRCOL_MAX_RUNTIME_LENGTH, pos=5),
        }

        runtable = self._all_get_or_create_table(
            where=self._overview_group, tablename="runs", description=rundescription_dict
        )

        hdf5_description_dict = {
            "complib": pt.StringCol(7, pos=0),
        ...
        hdf5_description_dict.update(
            {
                "purge_duplicate_comments": pt.BoolCol(pos=pos + 2),
                "results_per_run": pt.IntCol(pos=pos + 3),
                "derived_parameters_per_run": pt.IntCol(pos=pos + 4),
            }
        )

        hdf5table = self._all_get_or_create_table(
            where=self._overview_group, tablename="hdf5_settings", description=hdf5_description_dict
        )
    ...
    def _trj_store_explorations(self, traj):
        ...
        if not hasattr(self._overview_group, "explorations"):
            ...
            if explored_list:
                string_col = self._all_get_table_col(
                    "explorations", explored_list, "overview.explorations"
                )
            else:
                string_col = pt.StringCol(1)
            description = {"explorations": string_col}
            explorations_table = self._hdf5file.create_table(
                where=self._overview_group, name="explorations", description=description
            )
    ...
    def _srvc_make_overview_tables(self, tables_to_make, traj=None):
        ...
        for table_name in tables_to_make:
            ...
            paramdescriptiondict["location"] = pt.StringCol(
                pypetconstants.HDF5_STRCOL_MAX_LOCATION_LENGTH, pos=0
            )
            paramdescriptiondict["name"] = pt.StringCol(
                pypetconstants.HDF5_STRCOL_MAX_NAME_LENGTH, pos=1
            )
            ...
            if expectedrows > 0:
                paramtable = self._all_get_or_create_table(
                    where=self._overview_group,
    ...
    def _trj_store_trajectory(
        self, traj, only_init=False, store_data=pypetconstants.STORE_DATA, max_depth=None
    ):
        ...
        if not only_init:
            ...
            self._logger.info(
                f"Initialising storage or updating meta data of Trajectory `{self._trajectory_name}`."
            )
        ...
        if not traj._stored and self._trajectory_group is not None:
            raise RuntimeError(
                f"You want to store a completely new trajectory with name"
        ...
        if self._trajectory_group is None:
            self._trajectory_group = self._hdf5file.create_group(
                where="/",
        ...
        if store_data in (
            pypetconstants.STORE_DATA_SKIPPING,
        ...
        if hdf5_group is None:
            ...
            try:
                if location == "":
                    ...
                    hdf5_group = self._hdf5file.get_node(
                        where=self._trajectory_group, name=hdf5_location
                    )
            except pt.NoSuchNodeError:
                self._logger.debug(
                    f"Cannot store `{traj_node.v_name}` the parental hdf5 node with path `{hdf5_location}` does "
                    "not exist on disk."
                )

                if traj_node.v_is_leaf:
                    self._logger.error(
                        f"Cannot store `{traj_node.v_name}` the parental hdf5 "
                    ...
                    self._logger.debug(
                        "I will try to store the path from trajectory root to the child now."
                    )
        ...
        while loading_list:
            ...
            if isinstance(hdf5_group, pt.link.SoftLink):
                if with_links:
                    ...
                    self._tree_load_link(
                        parent_traj_node,
                        load_data=load_data,
                        traj=trajectory,
                        as_new=as_new,
                        hdf5_soft_link=hdf5_group,
                    )
                continue
            ...
            if is_leaf:
                ...
                self._prm_load_parameter_or_result(
                ...
                if in_trajectory:
                    ...
                    if HDF5StorageService.CLASS_NAME in hdf5_group._v_attrs:
                        class_name = self._all_get_from_attrs(
                            hdf5_group, HDF5StorageService.CLASS_NAME
                        )
                        class_constructor = trajectory._create_class(class_name)
                        instance = trajectory._construct_instance(class_constructor, name)
                        args = (instance,)
    ...
    def _tree_load_link(self, new_traj_node, load_data, traj, as_new, hdf5_soft_link):
        ...
        try:
            ...
            if link_name not in new_traj_node._links or load_data == pypetconstants.OVERWRITE_DATA:
                link_location = linked_group._v_pathname
                full_name = ".".join(link_location.split("/")[2:])
                if full_name not in traj:
                    self._tree_load_sub_branch(
                        traj,
                        full_name,
                ...
                if link_name not in new_traj_node._links:
                    new_traj_node._nn_interface._add_generic(
                        new_traj_node,
                        type_name=nn.LINK,
                        group_type_name=nn.GROUP,
                        args=(link_name, traj.f_get(full_name)),
                        kwargs={},
                        add_prefix=False,
                        check_naming=False,
                    )
            ...
            self._logger.error(
                f"Link `{hdf5_soft_link._v_name}` under `{new_traj_node.v_full_name}` is broken, cannot load it, "
                "I will ignore it, you have to "
                "manually delete it!"
            )

    def _tree_store_nodes_dfs(
        self,
        parent_traj_node,
        name,
        store_data,
        ...
        while store_list:
            ...
            if not hasattr(parent_hdf5_group, name):
                newly_created = True
                new_hdf5_group = self._hdf5file.create_group(
                    where=parent_hdf5_group, name=name, filters=self._all_get_filters()
                )
            else:
                newly_created = False
            ...
            if traj_node.v_is_leaf:
                self._prm_store_parameter_or_result(
                    traj_node,
                    store_data=store_data,
    ...
    def _tree_store_link(self, node_in_traj, link, hdf5_group):
        ...
        try:
            ...
            self._logger.debug(
                f"Could not store link `{link}` under `{node_in_traj.v_full_name}` immediately, "
            ...
            to_link_hdf5_group = self._hdf5file.get_node(where=linking_name)
        self._hdf5file.create_soft_link(where=hdf5_group, name=link, target=to_link_hdf5_group)

    ######################## Storing a Single Run ##########################################

    def _srn_store_single_run(
        self, traj, recursive=True, store_data=pypetconstants.STORE_DATA, max_depth=None
    ):
    ...
    def _all_store_param_or_result_table_entry(self, instance, table, flags, additional_info=None):
        ...
        if flags == (HDF5StorageService.ADD_ROW,):
            ...
            condvars = {
                "namecol": table.cols.name,
                "locationcol": table.cols.location,
                "name": name,
                "location": location,
            }
        ...
        self._all_add_or_modify_row(
            fullname, insert_dict, table, condition=condition, condvars=condvars, flags=flags
        )
    ...
    def _all_get_or_create_table(self, where, tablename, description, expectedrows=None):
        ...
        if tablename not in where_node:
            if expectedrows is not None:
                table = self._hdf5file.create_table(
                    where=where_node,
                    name=tablename,
    ...
    def _all_set_attributes_to_recall_natives(data, ptitem, prefix):
        ...
        if type(data) is tuple:
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_TUPLE
            )

        elif type(data) is list:
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_LIST
            )

        elif type(data) is np.ndarray:
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_NDARRAY
            )

        elif type(data) is np.matrix:
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_MATRIX
            )

        elif type(data) in pypetconstants.PARAMETER_SUPPORTED_DATA:
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_SCALAR
            )

            strtype = type(data).__name__

            if strtype not in pypetconstants.PARAMETERTYPEDICT:
                raise TypeError(
                    f"I do not know how to handle `{data}` its type is `{type(data)!r}`."
                )

            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.SCALAR_TYPE, strtype
            )

        elif type(data) is dict:
            if len(data) > 0:
                HDF5StorageService._all_set_attr(
                    ptitem, prefix + HDF5StorageService.COLL_TYPE, HDF5StorageService.COLL_DICT
                )
            else:
                HDF5StorageService._all_set_attr(
                    ptitem,
                    prefix + HDF5StorageService.COLL_TYPE,
        ...
        if type(data) in (list, tuple):
            ...
            if len(data) > 0:
                ...
                if strtype not in pypetconstants.PARAMETERTYPEDICT:
                    raise TypeError(
                        f"I do not know how to handle `{data}` its type is `{strtype}`."
                    )
            ...
            HDF5StorageService._all_set_attr(
                ptitem, prefix + HDF5StorageService.SCALAR_TYPE, str.__name__
            )
    ...
    def _all_add_or_modify_row(
        ...
        if (
            HDF5StorageService.MODIFY_ROW in flags or HDF5StorageService.ADD_ROW in flags
        ) and HDF5StorageService.REMOVE_ROW in flags:
        ...
        if row is None and HDF5StorageService.ADD_ROW in flags:
            ...
            raise ValueError(
                "Something is wrong, you might not have found "
        ...
        if HDF5StorageService.REMOVE_ROW not in flags and row is None:
            raise RuntimeError(
                f"Could not add or modify entries of `{item_name}` in table {table._v_name}"
            )
    ...
    def _all_extract_insert_dict(self, item, colnames, additional_info=None):
        ...
        if "comment" in colnames:
            comment = self._all_cut_string(
                item.v_comment.encode("utf-8"),
                pypetconstants.HDF5_STRCOL_MAX_COMMENT_LENGTH,
                self._logger,
            )
    ...
    def _all_cut_string(string, max_length, logger):
        ...
        if len(string) > max_length:
            logger.debug(
                f"The string `{string}` was too long I truncated it to {max_length} characters"
            )
            string = string[0 : max_length - 3] + b"..."

        return string

    def _all_create_or_get_group(self, name, parent_hdf5_group=None):
        """Creates or returns a group"""
        if name not in parent_hdf5_group:
            new_hdf5_group = self._hdf5file.create_group(
                where=parent_hdf5_group, name=name, title=name, filters=self._all_get_filters()
            )
    ...
    def _grp_store_group(
        ...
        if store_data == pypetconstants.STORE_NOTHING:
            ...
            self._logger.debug(
                f"Already found `{traj_group.v_full_name}` on disk I will not store it!"
            )
        elif not recursive:
            if _hdf5_group is None:
            ...
            if (_newly_created or overwrite) and type(traj_group) not in (
                nn.NNGroupNode,
                nn.ConfigGroup,
                nn.ParameterGroup,
                nn.DerivedParameterGroup,
                nn.ResultGroup,
            ):
                # We only store the name of the class if it is not one of the standard groups,
                # that are always used.
                setattr(
                    _hdf5_group._v_attrs,
            ...
            if key not in flags_dict:
                ...
                if (dtype is np.ndarray or dtype is dict) and len(data) == 0:
                    ...
                    try:
                        ...
                        raise pex.NoSuchServiceError(
                            f"I cannot store `{key}`, I do not understand thetype `{dtype}`."
                        )
        ...
        try:
            ...
            if row is None:
                self._all_store_param_or_result_table_entry(
                    ...
                    table,
                    flags=(HDF5StorageService.ADD_ROW,),
                    additional_info={"hexdigest": hexdigest},
                )
    ...
    def _prm_add_meta_info(self, instance, group, overwrite=False):
        ...
        if (
            not self._purge_duplicate_comments or definitely_store_comment
        ) and instance.v_comment != "":
    ...
    def _prm_store_from_dict(self, fullname, store_dict, hdf5_group, store_flags, kwargs):
        ...
        for key, data_to_store in store_dict.items():
            ...
            if "." in key:
                ...
                for inner_key in split_key:
                    ...
                    if newly_created:
                        setattr(
                            hdf5_group._v_attrs,
                            HDF5StorageService.STORAGE_TYPE,
                            HDF5StorageService.NESTED_GROUP,
                        )
                    else:
                        store_type = self._all_get_from_attrs(
                            hdf5_group, HDF5StorageService.STORAGE_TYPE
                        )
                        if store_type != HDF5StorageService.NESTED_GROUP:
                            raise ValueError(
                                f"You want to nested results but `{hdf5_group._v_name}` is already "
            ...
            if flag == HDF5StorageService.TABLE:
                ...
                HDF5StorageService.CARRAY,
                HDF5StorageService.EARRAY,
                HDF5StorageService.VLARRAY,
            ):
                self._prm_write_into_other_array(
                    key, data_to_store, hdf5_group, fullname, flag=flag, **kwargs
                )
            elif flag in (
                ...
                self._prm_write_pandas_data(
                    key, data_to_store, hdf5_group, fullname, flag, **kwargs
                )
    ...
    def _prm_store_parameter_or_result(
        self,
        instance,
        store_data=pypetconstants.STORE_DATA,
        store_flags=None,
        ...
        **kwargs,
        ...
        if store_data == pypetconstants.STORE_NOTHING:
            ...
            self._logger.debug(
                f"Already found `{instance.v_full_name}` on disk I will not store it!"
            )
            return
        ...
        try:
            ...
            if overwrite:
                ...
                if overwrite is True:
                    ...
                    self._all_delete_parameter_or_result_or_group(
                        instance, delete_only=to_delete, _hdf5_group=_hdf5_group
                    )
                    ...
                    if overwrite != "v_annotations" and len(stuff_not_to_be_overwritten) > 0:
                        self._logger.warning(
                            f"Cannot overwrite `{stuff_not_to_be_overwritten}`, these items are not supposed to "
                    ...
                    if len(stuff_to_overwrite) > 0:
                        self._all_delete_parameter_or_result_or_group(
                            instance, delete_only=list(stuff_to_overwrite)
                        )
                else:
                    raise ValueError(
                        f"Your value of overwrite `{overwrite}` is not understood. "
    ...
    def _shared_write_shared_data(self, key, hdf5_group, full_name, **kwargs):
        try:
            ...
            if flag == HDF5StorageService.TABLE:
                self._prm_write_shared_table(key, hdf5_group, full_name, **kwargs)
                ...
                self._prm_write_shared_array(key, data, hdf5_group, full_name, flag, **kwargs)

            hdf5data = hdf5_group._f_get_child(key)
            setattr(
                hdf5data._v_attrs, HDF5StorageService.STORAGE_TYPE, HDF5StorageService.SHARED_DATA
            )
    ...
    def _prm_write_shared_array(self, key, data, hdf5_group, full_name, flag, **kwargs):
        ...
        if flag == HDF5StorageService.ARRAY:
            ...
            raise RuntimeError(
                f"Flag `{flag}` of hdf5 data `{key}` of `{full_name}` not understood"
            )
    ...
    def _prm_write_shared_table(self, key, hdf5_group, fullname, **kwargs):
        ...
        table = self._hdf5file.create_table(
            where=hdf5_group, name=key, description=description, filters=filters, **kwargs
        )
        table.flush()
    ...
    def _prm_write_dict_as_table(self, key, data_to_store, group, fullname, **kwargs):
        ...
        if key in group:
            raise ValueError(
                f"Dict `{key}` already exists in `{fullname}`. Appending is not supported (yet)."
            )
        ...
        self._all_set_attributes_to_recall_natives(
            temp_dict, new_table, HDF5StorageService.DATA_PREFIX
        )

        setattr(new_table._v_attrs, HDF5StorageService.STORAGE_TYPE, HDF5StorageService.DICT)
    ...
    def _prm_write_pandas_data(self, key, data, group, fullname, flag, **kwargs):
        ...
        try:
            if "filters" not in kwargs:
                self._logger.debug("filters are no longer supported by pandas")
            ...
            if "expectedrows" in kwargs:
                self._logger.debug(
                    "expectedrows no longer supported by pandas, will ignore the option"
                )
            ...
            if isinstance(data, DataFrame):
                str_cols = [c for c in data.columns if isinstance(data[c].dtype, StringDtype)]
                if str_cols:
                    data = data.copy()
                    for c in str_cols:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    other_array = factory(
                        where=group, name=key, obj=conv_data, filters=filters, **kwargs
                    )
                except Exception:
            ...
            if data is not None:
                ...
                self._all_set_attributes_to_recall_natives(
                    data, other_array, HDF5StorageService.DATA_PREFIX
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    array = self._hdf5file.create_array(
                        where=group, name=key, obj=conv_data, **kwargs
                    )
                except Exception:
                    # Re-raise original error
                    raise exc
            ...
            if data is not None:
                # Remember the types of the original data to recall them on loading
                self._all_set_attributes_to_recall_natives(
                    data, array, HDF5StorageService.DATA_PREFIX
                )
    ...
    def _all_delete_parameter_or_result_or_group(
        self, instance, delete_only=None, remove_from_item=False, recursive=False, _hdf5_group=None
    ):
        ...
        if delete_only is None:
            if instance.v_is_group and not recursive and len(_hdf5_group._v_children) != 0:
                raise TypeError(
                    f"You cannot remove the group `{instance.v_full_name}`, it has children, please "
            ...
            for delete_item in delete_only:
                if (
                    remove_from_item
                    and hasattr(instance, "__contains__")
                    and hasattr(instance, "__delattr__")
                    and delete_item in instance
                ):
                ...
                try:
                    ...
                    self._logger.warning(
                        f"Could not delete `{delete_item}` from `{instance.v_full_name}`. HDF5 node not found!"
                    )
    ...
    def _prm_write_into_pytable(self, tablename, data, hdf5_group, fullname, **kwargs):
        ...
        try:
            ...
            if len(description_dict) > ptpa.MAX_COLUMNS:
                ...
                new_table_group = self._hdf5file.create_group(
                    where=hdf5_group, name=tablename, filters=self._all_get_filters(kwargs.copy())
                )
            ...
            for idx, descr_dict in enumerate(description_dicts):
                ...
                if idx == 0 and len(description_dict) <= ptpa.MAX_COLUMNS:
                    ...
                    setattr(
                        table._v_attrs, HDF5StorageService.STORAGE_TYPE, HDF5StorageService.TABLE
                    )
    ...
    def _all_get_table_col(self, key, column, fullname):
        ...
        try:
            ...
            self._logger.error(
                f"Failure in storing `{key}` of Parameter/Result `{fullname}`."
    ...
    def _prm_load_into_dict(
        ...
        for node in hdf5_group:
            ...
            if load_type == HDF5StorageService.DICT:
                ...
                raise pex.NoSuchServiceError(
                    f"Cannot load {full_name}, do not understand the hdf5 file "
    ...
    def _prm_load_parameter_or_result(
        self,
        instance,
        load_data=pypetconstants.LOAD_DATA,
        load_only=None,
        ...
        if load_data == pypetconstants.OVERWRITE_DATA:
            if instance.v_is_parameter and instance.v_locked:
                self._logger.debug(
                    f"Parameter `{instance.v_full_name}` is locked, I will skip loading."
                )
                return
            instance.f_empty()
        ...
        if load_data == pypetconstants.LOAD_SKELETON:
            ...
            if load_except is not None:
                raise ValueError(
                    "Please use either `load_only` or `load_except` and not both at the same time."
                )
            ...
            self._logger.debug(
                f"I am in load except mode, I will load everything except {load_except}."
            )
        ...
        if load_only is not None:
            ...
            if len(load_only) > 0:
                self._logger.warning(
                    f"You marked {load_only} for load only, "
                    f"but I cannot find these for `{full_name}`"
                )
        elif load_except is not None:
            if len(load_except) > 0:
                self._logger.warning(
                    f"You marked `{load_except}` for not loading, but these were not part "
        ...
        if load_dict:
            try:
                ...
                self._logger.error(f"Error while reconstructing data of leaf `{full_name}`.")
                raise
    ...
    def _prm_read_shared_data(self, shared_node, instance):
        ...
        try:
            ...
            self._logger.error(
                f"Failed loading `{shared_node._v_name}` of `{instance.v_full_name}`."
            )
            raise
    ...
    def _prm_read_table(self, table_or_group, full_name):
        ...
        try:
                ...
                for sub_table in table_or_group:
                    ...
                    for colname in sub_table.colnames:
                        ...
                        for idx, data in enumerate(data_list):
                            ...
                            data, type_changed = self._all_recall_native_type(
                                data, PTItemMock(data_type_dict), prefix
                            )
                            if type_changed:
                                data_list[idx] = data
                            else:
            ...
            self._logger.error(f"Failed loading `{table_or_group._v_name}` of `{full_name}`.")
            raise
    ...
    def _svrc_read_array(array):
        ...
        try:
            if (
                res.shape == (1,)
                and res[0] == empty_array_val
                and EMPTY_ARRAY_FIX_PT_2 in array._v_attrs
            ):
    ...
    def _prm_read_array(self, array, full_name):
        ...
        try:
            ...
            result, dummy = self._all_recall_native_type(
                result, array, HDF5StorageService.DATA_PREFIX
            )
    ...
    def _hdf5_interact_with_data(self, path_to_data, item_name, request, args, kwargs):
        ...
        if request == "create_shared_data" or request == "pandas_put":
            return self._shared_write_shared_data(
                key=item_name, hdf5_group=hdf5_group, full_name=path_to_data, **kwargs
            )
