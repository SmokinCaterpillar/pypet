...
class StorageTest(TrajectoryComparator):
    ...
    def test_storage_service_errors(self):
        ...
        with self.assertRaises(RuntimeError):
            traj.v_storage_service.store(
                "LIST", [("LEAF", None, None, None, None)], trajectory_name=traj.v_name
            )
    ...
    def test_store_overly_long_comment(self):
        ...
        traj = Trajectory(name="traj", add_time=True, filename=filename)
        traj.f_add_result("iii", 42, 43, comment=7777 * "6")
        traj.f_store()
    ...
    def test_maximum_overview_size(self):
        ...
        env = Environment(
            trajectory="Testmigrate", filename=filename, log_config=get_log_config(), add_time=True
        )
    ...
    def test_migrations(self):
        ...
        traj = Trajectory(name="Testmigrate", filename=make_temp_dir("migrate.hdf5"), add_time=True)
    ...
    def test_wildcard_search(self):
        ...
        traj = Trajectory(
            name="Testwildcard", filename=make_temp_dir("wilcard.hdf5"), add_time=True
        )
    ...
    def test_store_and_load_large_dictionary(self):
        traj = Trajectory(
            name="Testlargedict", filename=make_temp_dir("large_dict.hdf5"), add_time=True
        )
        ...
        traj2 = Trajectory(filename=make_temp_dir("large_dict.hdf5"), add_time=True)
    ...
    def test_auto_load(self):
        ...
        traj = Trajectory(
            name="Testautoload", filename=make_temp_dir("autoload.hdf5"), add_time=True
        )
    ...
    def test_get_default(self):
        ...
        traj = Trajectory(
            name="Testgetdefault", filename=make_temp_dir("autoload.hdf5"), add_time=True
        )
    ...
    def test_version_mismatch(self):
        traj = Trajectory(
            name="TestVERSION", filename=make_temp_dir("testversionmismatch.hdf5"), add_time=True
        )
        ...
        traj2 = Trajectory(
            name=traj.v_name, add_time=False, filename=make_temp_dir("testversionmismatch.hdf5")
        )
    ...
    def test_no_run_information_loading(self):
        ...
        traj = Trajectory(name="TestDelete", filename=filename, add_time=True)
    ...
    def test_delete_whole_subtrees(self):
        ...
        traj = Trajectory(
            name="TestDelete", filename=filename, large_overview_tables=True, add_time=True
        )

        traj.f_add_result("mytest.yourtest.test", a="b", c="d")
        traj.f_add_derived_parameter("mmm.gr.dpdp", 666)

        traj.f_add_result("hhh.ll", a="b", c="d")
        traj.f_add_derived_parameter("hhh.gg", 555)
    ...
    def test_delete_links(self):
        traj = Trajectory(
            name="TestDelete", filename=make_temp_dir("testpartiallydel.hdf5"), add_time=True
        )
    ...
    def test_overwrite_stuff(self):
        traj = Trajectory(
            name="TestOverwrite", filename=make_temp_dir("testowrite.hdf5"), add_time=True
        )
    ...
    def test_loading_as_new(self):
        ...
        traj = load_trajectory(
            name=traj.v_name, filename=filename, as_new=True, new_name="TestTraj", add_time=False
        )
    ...
    def test_partial_loading(self):
        traj = Trajectory(
            name="TestPartial", filename=make_temp_dir("testpartially.hdf5"), add_time=True
        )

        traj.f_add_result("mytest.test", a="b", c="d")
    ...
    def test_hdf5_settings_and_context(self):
        ...
        with Environment(
            ...
            table = (
                hdf5file.root._f_get_child(traj.v_name)
                ._f_get_child("overview")
                ._f_get_child("hdf5_settings")
            )
            ...
            self.assertTrue(row["complib"] == b"zlib")

            self.assertTrue(row["shuffle"])
            self.assertTrue(row["fletcher32"])
            self.assertTrue(row["pandas_format"] == b"t")
    ...
    def test_store_items_and_groups(self):
        ...
        traj = Trajectory(
            name="testtraj", filename=make_temp_dir("teststoreitems.hdf5"), add_time=True
        )

        traj.f_store()

        traj.f_add_parameter(
            "group1.test", 42, comment="TooLong" * pypetconstants.HDF5_STRCOL_MAX_COMMENT_LENGTH
        )
        ...
        traj2 = Trajectory(
            name=traj.v_name, add_time=False, filename=make_temp_dir("teststoreitems.hdf5")
        )
