...
from pypet.tests.testutils.ioutils import make_temp_dir, parse_args, run_suite
...
class ParameterTest(TrajectoryComparator):
    ...
    def test_store_load_with_hdf5(self):
        ...
        new_traj = Trajectory(
            name=traj_name, dynamic_imports=self.dynamic_imports, filename=filename
        )
    ...
    def test_type_error_for_get_item(self):
        for name, param in self.param.items():
            if name not in self.explore_dict:
                with self.assertRaises(TypeError):
                    param[1]
    ...
    def test_the_insertion_made_implicetly_in_setUp(self):
        for key, val in self.data.items():
            if key not in self.explore_dict:
    ...
    def test_expanding(self):
        for name, param in self.param.items():
            if name in self.explore_dict:
                ...
                self.assertTrue(
                    len(param) == 2 * len(self.explore_dict[name]),
    ...
    def test_exploration(self):
        for key, vallist in self.explore_dict.items():
            ...
            for idx, val in enumerate(vallist):
                ...
                (
                    self.assertTrue(np.all(repr(param.f_get()) == repr(val))),
                    f"{param.f_get()} != {val}",
                )
    ...
    def test_storage_and_loading(self):
        ...
        for key, param in self.param.items():
            ...
            if param.f_has_range():
                if isinstance(param, (ArrayParameter, PickleParameter)) and not isinstance(
...
class PickleParameterTest(ParameterTest):
    ...
    def make_params(self):
        ...
        self.protocols = {}
        for key, val in self.data.items():
            self.param[key] = PickleParameter(
                self.location + "." + key, val, comment=key, protocol=count
            )
    ...
    def test_meta_settings(self):
        for key, param in self.param.items():
            ...
            self.assertEqual(
                param.v_protocol,
                self.protocols[key],
                f"{param.v_protocol} != {self.protocols[key]}",
            )
...
class SparseParameterTest(ParameterTest):
    ...
    def explore(self):
        ...
        self.explore_dict = cartesian_product(
            ...
            (("npstr", "val0"), ("spsparse_csr", "spsparse_csc", "spsparse_bsr", "spsparse_dia")),
        )
...
class ResultTest(TrajectoryComparator):
    ...
    def make_results(self):
        self.results = {}
        self.results["test.res.on_constructor"] = self.Constructor(
            "test.res.on_constructor", **self.data
        )
    ...
    def setUp(self):
        ...
        self.data["numpy_array"] = np.array([[3232.3, 323232323232.32323232], [4.0, 4.0]])
        ...
        myframe = pd.DataFrame(
            data={"TC1": [1, 2, 3], "TC2": ["Waaa", np.nan, ""], "TC3": [1.2, 42.2, np.nan]}
        )
        ...
        for key, val1 in self.data.items():
            ...
            if isinstance(val1, dict):
                for innerkey in val1:
                    ...
                    self.assertEqual(repr(innerval1), repr(innerval2), f"{val1} != {val2}")
            else:
...
class PickleResultTest(ResultTest):
    ...
    def make_results(self):
        self.results = {}
        self.results["test.res.on_constructor"] = self.Constructor(
            "test.res.on_constructor", protocol=0, **self.data
        )
        ...
        self.protocols = {"test.res.on_constructor": 0, "test.res.args": 1, "test.res.kwargs": 2}
