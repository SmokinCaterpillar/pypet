...
from pypet import BaseResult, Environment, pypetconstants
from pypet.naturalnaming import NNGroupNode, ParameterGroup
...
from pypet.tests.testutils.ioutils import get_log_config, make_temp_dir, parse_args, run_suite
...
class TrajectoryTest(unittest.TestCase):
    ...
    def test_kids(self):
        ...
        self.assertEqual(set(self.traj._children.keys()), set(dir(self.traj._kids)))
    ...
    def test_get_data_dictionaries_directly(self):
        ...
        self.assertTrue(
            comp.nested_equal(config_dict_directly, config_dict_from_subtree),
        ...
        self.assertTrue(
            comp.nested_equal(parameters_dict_directly, parameters_dict_from_subtree),
        ...
        self.assertTrue(
            comp.nested_equal(
                derived_parameters_dict_directly, derived_parameters_dict_from_subtree
            ),
        ...
        with self.assertRaises(ValueError):
            derived_parameters_dict_directly = self.traj.f_get_derived_parameters(
                copy=False, fast_access=True
            )

        derived_parameters_dict_directly = self.traj.f_get_derived_parameters(
            copy=True, fast_access=True
        )
        ...
        self.assertTrue(
            comp.nested_equal(results_dict_directly, results_dict_from_subtree),
        ...
        for key in self.explore_dict:
            self.assertTrue(
                comp.nested_equal(
                    self.traj.f_get(key), explore_dict_directly[self.traj.f_get(key).v_full_name]
                )
            )
        ...
        with self.assertRaises(ValueError):
            explore_dict_directly = self.traj.f_get_explored_parameters(
                copy=False, fast_access=True
            )
        ...
        for key in self.explore_dict:
            self.assertTrue(
                comp.nested_equal(
                    self.traj.f_get(key, fast_access=True),
    ...
    def test_f_get(self):
        ...
        self.assertTrue(
            self.traj.f_get("FloatParam").f_get() == 4.0,
            f"{self.traj.f_get('FloatParam').f_get()} != 4.0",
        )
            ...
            if not item.v_is_leaf:
                if as_run in item._children:
                    ...
                    for child in item._children.values():
                        if not (
                            child.v_name.startswith(pypetconstants.RUN_NAME)
                            and child.v_name != pypetconstants.RUN_NAME_DUMMY
                        ):
    ...
    def test_find_in_all_runs_with_groups(self):
        ...
        res_dict = self.traj.f_get_from_runs(
            name="sub.resulttest2", include_default_run=False, use_indices=True
        )
    ...
    def test_standard_change_param_change(self):
        ...
        self.assertIsInstance(
            self.traj.f_get("normal"),
            ImAParameterInDisguise,
            f"Param is {type(self.traj.normal)} insted of ParamInDisguise.",
        )
    ...
    def test_get_children(self):
        ...
        for node in self.traj.f_iter_nodes():
            if not node.v_is_leaf:
                ...
                self.assertEqual(
                    sorted(node.f_get_children(copy=True).keys()), sorted(node._children.keys())
                )
    ...
    def test_to_nested_dict(self):
        ...
        lookout = {
            "parameters": {"my": {"param": 42, "fff": 44}},
            "results": {"runs": {"run_ALL": {"myresult": 55}}},
        }
        result = traj.f_to_dict(nested=True, fast_access=True)
        self.assertTrue(nested_equal(result, lookout), "{} != {}".format(result, lookout))
    ...
    def test_to_nested_dict_intermediate_node(self):
        ...
        lookout = {"runs": {"run_ALL": {"myresult": 55}}}
    ...
    def test_iter_leaves(self):
        ...
        all_leaves = (
            len(self.traj._config)
            + len(self.traj._parameters)
            + len(self.traj._results)
            + len(self.traj._derived_parameters)
...
class TrajectoryFindTest(unittest.TestCase):
    ...
    def test_complex_find_statement(self):
        pred = lambda x, ar, z, scalar: (
            scalar == 42
            and (x == 2 or x == 4 or x == 5)
            and (z == "meter" or z == "berserker")
            and np.all(np.array([4, 5, 6]) == ar)
        )
...
class SingleRunTest(unittest.TestCase):
    ...
    def test_auto_prepend(self):
        ...
        self.assertEqual(
            self.single_run.f_get("dpar.huhu.test", shortcuts=False, fast_access=True), 990
        )
