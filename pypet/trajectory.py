...
from pypet.utils.decorators import (
    copydoc,
    kwargs_api_change,
    kwargs_mutual_exclusive,
    not_in_run,
)
from pypet.utils.helpful_functions import format_time, is_debug
    ...
    * Results:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not copy_traj:
            ...
            if faulty_names:
                raise ValueError(
                    f"Your Trajectory {name} f_contains the following not admissible names: "
                    f"{faulty_names} please choose other names."
                )

            internal_wildcard_functions = {
                ("$", "crun"): make_run_name,
            ...
            self._storage_service, unused_kwargs = storage_factory(
                storage_service=storage_service, trajectory=self, **kwargs
            )
            if len(unused_kwargs) > 0:
                raise ValueError(
                    f"The following keyword arguments were not used: `{unused_kwargs}`"
                )
    ...
    def f_set_properties(self, **kwargs):
        ...
        for name in kwargs:
            ...
            if not name.startswith("v_"):
                name = "v_" + name
            if name not in self._nn_interface._not_admissible_names:
    ...
    def f_set_crun(self, name_or_idx):
        ...
        *
        ...
        """
        if name_or_idx is None or name_or_idx == self.f_wildcard("$", -1) or name_or_idx == -1:
    ...
    def f_shrink(self, force=False):
        ...
        if self._stored and not force:
            raise TypeError(
                "Your trajectory is already stored to disk or database, shrinking is not allowed."
            )
        ...
        for param in self._explored_parameters.values():
            ...
            try:
                ...
                self._logger.error(
                    "Could not shrink `%s` because of:`%s`" % (param.v_full_name, repr(exc))
                )
    ...
    def _preset(self, name, args, kwargs):
        ...
        if self.f_contains(name, shortcuts=False):
            raise ValueError(
                f"Parameter `{name}` is already part of your trajectory, use the normal"
                        ...
                        try:
                            ...
                            if already_found:
                                raise pex.NotUniqueNodeError(
                                    f"`{name}` has been found several times in one run."
                                )
                            else:
                                already_found = True
    ...
    def f_expand(self, build_dict, fail_safe=True):
        ...
        if len(self._explored_parameters) == 0:
            self._logger.info(
                "Your trajectory has not been explored, yet. I will call `f_explore` instead."
            )
            return self.f_explore(build_dict)

        enlarge_set = set([self.f_get(key).v_full_name for key in build_dict.keys()])
        ...
        if not set(self._explored_parameters.keys()) == enlarge_set:
            raise TypeError(
                "You have to enlarge dimensions you have explored before! Currently"
        ...
        if any(x is None for x in self._explored_parameters.values()):
            raise TypeError(
                "At least one of your explored parameters is not fully loaded, please load it."
            )
        ...
        if fail_safe:
            ...
            try:
                ...
                self._logger.error(
                    "Cannot deepcopy old parameter ranges, if "
    ...
    def _test_run_addition(self, length):
        ...
        if not len(self) == length:
            raise RuntimeError(
                "Your trajectory has a wrong length. Your naming function does not work!"
            )
        dummy = self.f_wildcard("$", -1)
        if dummy in self._run_information:
            raise RuntimeError(
                "Your renaming function does not return an appropriate value for "
            ...
            if param._stored:
                try:
                    ...
                    self._logger.exception(
                        "Could not delete expanded parameter `%s` from disk." % param.v_full_name
                    )

    def __copy__(self):
        """Returns a shallow copy"""
        return self.f_copy(copy_leaves=True, with_links=True)

    def f_copy(self, copy_leaves=True, with_links=True):
        ...
        new_traj._run_by_environment = (
            self._run_by_environment
        )  # To disable manual running of experiment
        ...
        new_traj._copy_from(self, copy_leaves=copy_leaves, with_links=with_links)

        # Copy references to new nodes and leaves
        for my_dict, new_dict in (
            (self._new_nodes, new_traj._new_nodes),
            ...
            for key in my_dict:
                ...
                if parent is self:
                    ...
                    new_parent = new_traj.f_get(
                        parent.v_full_name, shortcuts=False, with_links=False, auto_load=False
                    )
    ...
    def _copy_from(self, node, copy_leaves=True, overwrite=False, with_links=True):
        ...
        def _add_leaf(leaf):
            ...
            try:
                found_leaf = self.f_get(
                    leaf_full_name, with_links=False, shortcuts=False, auto_load=False
                )
                if overwrite:
                ...
                pass
            if copy_leaves is True or (
                copy_leaves == "explored" and leaf.v_is_parameter and leaf.v_explored
            ):
        ...
        def _add_group(group):
            ...
            try:
                found_group = self.f_get(
                    group_full_name, with_links=False, shortcuts=False, auto_load=False
                )
                if overwrite:
            ...
            if node.v_is_leaf:
                ...
                if with_links:
                    for current in has_links:
                        mine = self.f_get(
                            current.v_full_name, with_links=False, shortcuts=False, auto_load=False
                        )
    ...
    def _add_run_info(
        ...
        name="",
        timestamp=42.0,
        finish_timestamp=1.337,
        runtime="forever and ever",
        time=">>Maybe time`s gone on strike",
        completed=0,
        parameter_summary="Not yet my friend!",
    ...
    def _check_if_both_have_same_parameters(self, other_trajectory, ignore_data, consecutive_merge):
        """Checks if two trajectories live in the same space and can be merged."""

        if not isinstance(other_trajectory, Trajectory):
            raise TypeError(
                "Can only merge trajectories, the other trajectory"
        ...
        if diff:
            raise TypeError(
                f"The wildcard sets are not matching. `{wildcard_set}` != `{other_wildcard_set}`"
            )
        ...
        if other_trajectory._stored:
            with self._nn_interface._disable_logging:
                other_trajectory.f_load_items(
                    other_trajectory._parameters.keys(), only_empties=True
                )
        ...
        if diff:
            ...
            for full_name in diff:
                ...
                if not any(
                    x in self._run_information
                    or x in other_trajectory._run_information
                    or x in run_dummys
                    for x in split_name
                ):
        ...
        for key, other_param in allotherparams.items():
            ...
            split_key = key.split(".")
            if any(
                x in self._run_information or x in other_trajectory._run_information
                for x in split_key
            ):
                pass
            else:
                if not my_param._values_of_same_type(my_param.f_get(), other_param.f_get()):
                    raise TypeError(
                        f"Cannot merge trajectories, values of parameters `{key}` are not "
    ...
    def f_backup(self, **kwargs):
        ...
        self._storage_service.store(
            pypetconstants.BACKUP, self, trajectory_name=self.v_name, **kwargs
        )
    ...
    def _make_reversed_wildcards(self, old_length=-1):
        ...
        for wildcards, func in self._wildcard_functions.items():
            for irun in range(start, len(self)):
                ...
                if translated_name not in self._reversed_wildcards:
    ...
    def f_merge(
        ...
        if consecutive_merge and trial_parameter is not None:
            self._logger.warning(
                "If you do a consecutive merge and specify a trial parameter, "
                "your merging will still suffer from quadratic time complexity!"
            )
        if consecutive_merge and backup:
            self._logger.warning(
                "If you do a consecutive merge and backup, "
                "your merging will still suffer from quadratic time complexity!"
            )
        ...
        used_runs, changed_parameters = self._merge_parameters(
            other_trajectory, remove_duplicates, trial_parameter, ignore_data
        )
        ...
        if len(used_runs) == 0:
            raise ValueError(
                "Your merge discards all runs of the other trajectory, maybe you "
        ...
        allowed_translations = set(
            [
                translation
                for translation, pair in other_trajectory._reversed_wildcards.items()
                if any(x in used_runs for x in pair[0])
            ]
        )
        self._merge_single_runs(other_trajectory, used_runs)

        self._logger.info("Merging derived parameters")
        self._merge_derived_parameters(
        ...
        if not slow_merge:
            try:
                ...
                try:
                    ...
                    self._logger.warning(
                        "Could not determine the filename of the other "
                ...
                self._logger.exception(
                    "My storage service does not support merging of trajectories, "
        ...
        if keep_info:
            ...
            self.f_add_config(config_name, len(used_runs), comment="Number of merged runs")

            config_name = f"merge.{merge_name}.timestamp"
            self.f_add_config(config_name, timestamp, comment="Timestamp of merge")

            config_name = f"merge.{merge_name}.hexsha"
            self.f_add_config(config_name, hexsha, comment="SHA-1 identifier of the merge")

            config_name = f"merge.{merge_name}.remove_duplicates"
            self.f_add_config(
                config_name, remove_duplicates, comment="Option to remove duplicate entries"
            )

            if original_ignore_data:
                config_name = f"merge.{merge_name}.ignore_data"
                self.f_add_config(
                    config_name, tuple(original_ignore_data), comment="Data to ignore during merge"
                )

            config_name = f"merge.{merge_name}.length_before_merge"
            self.f_add_config(config_name, len(self), comment="Length of trajectory before merge")
            ...
            if self.v_version != VERSION:
                ...
                self.f_add_config(
                    config_name,
                    self.v_version,
                    comment="Pypet version if it differs from the version of the trajectory",
                )

            if trial_parameter is not None:
                config_name = f"merge.{merge_name}.trial_parameter"
                self.f_add_config(
                    config_name, len(other_trajectory), comment="Name of trial parameter"
                )
            ...
            if keep_other_trajectory_info:
                ...
                self.f_add_config(
                    config_name,
                    other_trajectory.v_timestamp,
                    comment="Timestamp of creation of other trajectory merged into the current one",
                )

                config_name = f"merge.{merge_name}.other_trajectory.length"
                self.f_add_config(
                    config_name, len(other_trajectory), comment="Length of other trajectory"
                )
    ...
    def _merge_single_runs(self, other_trajectory, used_runs):
        ...
        len(self)  # Variable to count the increasing new run indices and create
        # new run names
    ...
    def _merge_derived_parameters(
        self, other_trajectory, used_runs, rename_dict, allowed_translations, ignore_data
    ):
        ...
        for param_name in other_derived_parameters:
            ...
            new_param_name = self._rename_full_name(
                param_name, other_trajectory, used_runs=used_runs
            )
            if new_param_name in self:
                my_param = self.f_get(new_param_name, fast_access=False)
                if my_param._equal_values(my_param.f_get(), param.f_get()) and not (
                    my_param.f_has_range() or param.f_has_range()
                ):
                    continue

            first_new_param_name = self._rename_full_name(
                param_name, other_trajectory, new_run_idx=new_first_run_idx
            )
            ...
            for run_idx in used_runs.values():
                if run_idx == new_first_run_idx:
                    continue
                next_name = self._rename_full_name(
                    param_name, other_trajectory, new_run_idx=run_idx
                )
        ...
        for param_name in other_derived_parameters:
            ...
            ignore_data.add(param_name)
            if any(
                x in other_trajectory._reversed_wildcards and x not in allowed_translations
                for x in split_name
            ):
                continue
            ...
            if self.f_contains(new_name):
                ...
                if my_param._equal_values(my_param.f_get(), param.f_get()) and not (
                    ...
                    self._logger.error(
                        "Could not merge parameter `%s`. I will ignore it!" % new_name
                    )
        ...
        if len(linked_items) > 0:
            ...
            for old_linked_name in other_trajectory._linked_by:
                ...
                if any(x in run_name_dummys for x in split_name):
                    self._logger.warning(
                        "Ignoring all links linking to `%s` because "
                ...
                if all(x in allowed_translations for x in split_name):
                    new_linked_full_name = self._rename_full_name(
                        old_linked_name, other_trajectory, used_runs=used_runs
                    )
                else:
                ...
                for linking_node, link_set in old_link_dict.values():
                    ...
                    if any(x in run_name_dummys for x in split_name):
                        self._logger.warning(
                            "Ignoring links under `%s` because "
                            "I don`t know how to resolve links "
                            "under a `%s` node." % (linking_full_name, str(run_name_dummys))
                        )

                    split_name = linking_full_name.split(".")
                    if any(x in allowed_translations for x in split_name):
                        new_linking_full_name = self._rename_full_name(
                            linking_full_name, other_trajectory, used_runs=used_runs
                        )
                    else:
                    ...
                    for link in link_set:
                        ...
                        if link in run_name_dummys:
                            self._logger.warning(
                                "Ignoring link `%s` under `%s` because "
                        ...
                        try:
                            ...
                            if self.f_contains(new_linking_full_name):
                                new_linking_item = self.f_get(
                                    new_linking_full_name, shortcuts=False
                                )
                            else:
                            ...
                            if link not in new_linking_item._links:
                                new_linking_item.f_add_link(link, new_linked_item)
                            else:
                                self._logger.debug(
                                    "Link `%s` exists already under `%s`."
                                    % (link, new_linked_item.v_full_name)
                                )
                        except (AttributeError, ValueError) as exc:
                            self._logger.error(
                                "Could not copy link `%s` under `%s` linking "
                                "to `%s` due to `%s`"
                                % (link, linking_full_name, old_linked_name, repr(exc))
                            )
    ...
    def _merge_results(
        self, other_trajectory, rename_dict, used_runs, allowed_translations, ignore_data
    ):
        """Merges all results.
        ...
        for result_name in other_results:
            ...
            if any(
                ...
                continue
            new_name = self._rename_full_name(result_name, other_trajectory, used_runs=used_runs)
            if self.f_contains(new_name):
    ...
    def _merge_parameters(
        self, other_trajectory, remove_duplicates=False, trial_parameter_name=None, ignore_data=()
    ):
        ...
        if trial_parameter_name:
            ...
            if not isinstance(my_trial_parameter, BaseParameter):
                raise TypeError(
                    f"Your trial_parameter `{trial_parameter_name}` does not evaluate to a real parameter"
                    " in the trajectory"
                )
            ...
            if trial_parameter_name not in self._explored_parameters:
        ...
        set([f(-1) for f in other_trajectory._wildcard_functions.values()])
        ...
        for key in params_to_merge:
            ...
            if not my_param._values_of_same_type(my_param.f_get(), other_param.f_get()):
                raise TypeError(
                    f"The parameters with name `{key}` are not of the same type, cannot "
                    "merge trajectory."
                )
        ...
        for my_param, other_param in params_to_change.values():
            ...
            if fullname == trial_parameter_name:
                ...
                if other_param.f_has_range():
                    other_range = (
                        x
                        for jdx, x in enumerate(other_param.f_get_range(copy=False))
                        if jdx in used_runs
                    )
                else:
            ...
            if fullname not in self._explored_parameters:
    ...
    def f_migrate(self, new_name=None, in_store=False, new_storage_service=None, **kwargs):
        ...
        if new_storage_service is not None or len(kwargs) > 0:
            self._storage_service, unused_kwargs = storage_factory(
                storage_service=new_storage_service, trajectory=self, **kwargs
            )
        if len(unused_kwargs) > 0:
            raise ValueError(f"The following keyword arguments were not used: `{unused_kwargs}`")
    ...
    def f_store(self, only_init=False, store_data=pypetconstants.STORE_DATA, max_depth=None):
    ...
    def f_is_empty(self):
        ...
        return (
            ...
            and len(self._derived_parameters) == 0
            and len(self._results) == 0
            and len(self._other_leaves) == 0
        )
    ...
    def f_find_idx(self, name_list, predicate):
        ...
        if self._is_run and not self.v_full_copy:
            raise TypeError(
                "You cannot use this function during a multiprocessing signle run and "
        ...
        for name in name_list:
            ...
            if not param.v_is_parameter:
                raise TypeError(
                    f"`{name}` is not a parameter it is a {type(param)}, find idx is not applicable"
                )
    ...
    def _return_item_dictionary(param_dict, fast_access, copy):
        ...
        if not copy and fast_access:
            raise ValueError(
                "You cannot access the original dictionary and use fast access at the same time!"
            )
        if not fast_access:
            if copy:
    ...
    def f_to_dict(
        self, fast_access=False, short_names=False, nested=False, copy=True, with_links=True
    ):
        ...
        """
        return self._nn_interface._to_dict(
            self,
    ...
    def f_store_items(self, iterator, *args, **kwargs):
        ...
        if not self._stored:
            raise TypeError(
                "Cannot store stuff for a trajectory that has never been "
        ...
        if fetched_items:
            self._storage_service.store(
                pypetconstants.LIST, fetched_items, trajectory_name=self.v_name
            )
        else:
            raise ValueError(
                "Your storage was not successful, could not find a single item to store."
            )
    ...
    def f_load_items(self, iterator, *args, **kwargs):
        ...
        if fetched_items:
            ...
            self._logger.warning(
                "Your loading was not successful, could not find a single item to load."
            )
    ...
    def f_delete_links(self, iterator_of_links, remove_from_trajectory=False):
        ...
        try:
            self._storage_service.store(
                pypetconstants.LIST, to_delete_links, trajectory_name=self.v_name
            )
        except:
            self._logger.error(
                "Could not remove `%s` from the trajectory. Maybe the"
    ...
    def __dir__(self):
        ...
        if self._is_run:
            result = [
                x
                for x in result
                if not x.startswith("f_") or not getattr(getattr(self, x), "_not_in_run", False)
            ]
        if not is_debug():
